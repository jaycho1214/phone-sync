...
- Inject DiscoveryService dependency
- startServer() method:
  - Create and start PhoneSyncServer on port 0
  - Get the actual assigned port from server
  - Call discoveryService.advertise(deviceName, actualPort) to advertise on network
  - Update state to isRunning: true, port: actualPort
- stopServer() method:
  - Call discoveryService.stopAdvertising() first
  - Stop the HTTP server
  - Update state to isRunning: false
