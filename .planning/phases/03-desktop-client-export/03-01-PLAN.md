...
```bash
cd /Users/jay/Codes/jljm/jljm-phonesync/apps/desktop_client && flutter analyze
# Test on macOS (faster to build)
flutter run -d macos
```
App launches, shows discovery screen, can navigate to pairing.
  </verify>
  <done>Discovery screen shows devices, PIN entry has auto-advance, successful pairing navigates to home, session persists across restart.</done>
</task>
...
3. App launches on macOS: `flutter run -d macos`
4. Discovery screen appears on launch
5. Manual IP entry works when no devices found
6. PIN entry has 6 boxes with auto-advance
7. After pairing (with running Android server), app shows home screen
8. Restart app: auto-connects to paired device if online
</verification>
