...
must_haves:
  ...
  artifacts:
    ...
    - path: "apps/desktop_client/lib/services/phone_normalizer.dart"
      provides: "E.164 normalization for Korean and international numbers"
...
class PhoneEntries extends Table {
  // Normalized E.164 format phone number - primary key
...
- Method: getAllEntries({bool koreanMobileOnly = true})
  - If koreanMobileOnly: filter where phoneNumber LIKE '+8210%'
...
class PhoneNormalizer {
  ...
  /// Korean: +821012345678 (strip to digits, add +82, remove leading 0)
  ...
  String? normalize(String rawNumber) {
    ...
    // Korean domestic mobile (010-xxxx-xxxx)
...
No errors.
  ...
  <done>Phone normalizer converts Korean/international numbers to E.164, sync service fetches with progress, provider handles full sync flow with deduplication.</done>
</task>
...
- Method: export(deviceName)
  1. Get entries from repository (with filter)
...
flutter run -d macos
```
App builds, home screen shows, sync and export buttons work.
  </verify>
  <done>Excel export runs in isolate, filename follows format, home screen shows sync progress and export with Korean filter toggle.</done>
</task>
...
6. Korean mobile filter:
   - ON (default): only +8210* numbers
   - OFF: all numbers
7. 50k+ rows: UI remains responsive during export
8. App restart: data still in database, can export without re-sync
</verification>
